...
// rai-footer-exists.ts, with the same linear-time guarantees. Must stay
// identical to the Python plugin's pattern — enforced by
// test_signoff_pattern_parity_with_node_plugin in the Python suite.
const SIGNED_OFF_BY_PATTERN = new RegExp(
  String.raw`^Signed-off-by:[^\S\r\n]+[^\s<][^<\r\n]*(?<=\s)<[^>\r\n]+>\r?$`,
  'im',
);
